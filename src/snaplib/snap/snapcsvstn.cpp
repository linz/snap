...
SnapCsvStn::SnapCsvStn(network *net, const std::string &name, const OptionString &config) : SnapCsvBase(name, config),
                                                                                            _code("Station code"),
                                                                                            _name("Station name"),
...
void SnapCsvStn::loadDefinitionCommand(const string &command, RecordStream &rs)
{
    ...
    if (command == "code")
    {
        loadValueDefinition(rs, _code);
    }
    else if (command == "name")
    {
        loadValueDefinition(rs, _name);
    }
    else if (command == "longitude")
    {
        loadValueDefinition(rs, _crdlon);
    }
    else if (command == "easting")
    {
        loadValueDefinition(rs, _crdlon);
    }
    else if (command == "x")
    ...
    {
        loadValueDefinition(rs, _crdlat);
    }
    else if (command == "northing")
    {
        loadValueDefinition(rs, _crdlat);
    }
    else if (command == "y")
    ...
    {
        loadValueDefinition(rs, _crdhgt);
    }
    else if (command == "z")
    {
        loadValueDefinition(rs, _crdhgt);
    }
    else if (command == "geoid_undulation")
    {
        loadValueDefinition(rs, _crdund);
    }
    else if (command == "deflection_east")
    {
        loadValueDefinition(rs, _crdxi);
    }
    else if (command == "deflection_north")
    {
        loadValueDefinition(rs, _crdeta);
    }
    else if (command == "coordinate_system")
    {
        loadValueDefinition(rs, _crdsys);
    }
    else if (command == "height_type")
    {
        loadValueDefinition(rs, _hgttype);
    }
...
void SnapCsvStn::loadRecord()
{
    ...
    if (geodetic)
    {
        try
        {
            crdlon = parseAngle(_angleformat, _crdlon.value(), "EW");
        }
        catch (const ParseAngleException &e)
        {
            dataError(string("Badly formatted longitude: " + _crdlon.value()));
        }
        try
        {
            crdlat = parseAngle(_angleformat, _crdlat.value(), "NS");
        }
        catch (const ParseAngleException &e)
        {
            dataError(string("Badly formatted latitude: " + _crdlat.value()));
        }
    }
    else
    {
