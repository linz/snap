...
package LINZ::Geodetic::Util::GridFile;
...
sub WriteToFile
{
    ...
    {
        binmode($fh);
        use LINZ::Geodetic::Util::Packer;
        my $filebigendian = $fmt eq 'GRID1B' || $fmt eq 'GRID2B';
        my $packer = new LINZ::Geodetic::Util::Packer($filebigendian);
...
sub RowData {
   ...
   die "Invalid row in LINZ::Geodetic::Util::GridFile::RowData\n"
     if $row < 0 || $row >= $self->{ngridy};
   die "Invalid column in LINZ::Geodetic::Util::GridFile::RowData\n"
...
sub Convert
{
    ...
    die "Missing arguments for LINZ::Geodetic::Util::GridFile::Convert\n" if @args < 2;
    my $from = shift @args;
    my $to = shift @args;
    my %options;
    foreach my $a (@args)
    ...
    my $grid = new LINZ::Geodetic::Util::GridFile($from);
...
LINZ::Geodetic::Util::GridFile -- Reads, interpolates, writes LINZ grid files

=head1 SYNOPSIS

   use LINZ::Geodetic::Util::GridFile

   my $grid = new LINZ::Geodetic::Util::GridFile( filename )
   my $grid = newEmbedded LINZ::Geodetic::Util::GridFile( filehandle, offset )
   ...
   perl -MLINZ::Geodetic::Util::GridFile -e LINZ::Geodetic::Util::GridFile::Convert source.grd target.grd format=GRID2L
