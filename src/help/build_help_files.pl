...
Syntax: help_file_name base_directory ...  -x exclude_dir_name ..
...
foreach my $f (sort keys %contentsfiles )
{
   ...
   elsif( lc(&ContentsTitle($htmlfiles->{$f})) ne lc($contentsfiles{$f}) && $verbose )
   {
...
{
   print "File $f not in  contents\n" if ! exists $contentsfiles{$f} && $verbose;
}
