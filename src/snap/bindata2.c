...
void print_input_data( FILE *out )
{
    ...
    list_datatypes_used( out );

    fprintf(out,"\nFil/Lin  %-*s Height %-*s Height Type   Value      Error\n",
            stn_name_width,"From",stn_name_width,"To");
    ...
    while( get_bindata( b ) == OK )
    {
        ...
        case SURVDATA:
            ...
            switch( sd->format )
            {
            case SD_OBSDATA: list_obsdata( out, sd ); break;
            case SD_VECDATA: list_vecdata( out, sd ); break;
            case SD_PNTDATA: list_pntdata( out, sd ); break;
            default: program_error("Invalid survdata format",
                                       "print_input_data");
            }

        case NOTEDATA: break;

        default:      program_error("Invalid binary type","print_input_bindata");
            break;
        }
    }
