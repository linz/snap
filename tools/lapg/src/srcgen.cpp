...
void SourceGenerator::process_directive( char *id, char *value, int line, int column )
{
	if( !strcmp( id, "class" ) ) {
	...
	} else if( !strcmp( id, "parsefunc" ) ) {
		parsefunc = value;

	} else if( !strcmp( id, "parseargs" ) ) {
		parseargs = value;

	} else if( !strcmp( id, "errorfunc" ) ) {
		errorfunc = value;
		...
		ns = value;
                ...
                if( ! tmpl )
                {
                    error( 0, "lapg: %s, %i(%i) cannot open template file %s\n", sourcename,line,column,value);
                }
                else
                {
                    long l;
                    ...
                    if( l > 0 )
                    {
                        char *t = templatedata = new char[l+1];
                        *t = 0;
                        while( fgets(t, l, tmpl))
                        {
                            int nc = strlen(t);
                            t += nc;
                            l -= nc;
...
void SourceGenerator::process( int debug )
{
	...
	// init
	ns = errprefix = classn = getsym = errorfunc = parsefunc = parseargs = NULL;
	...
	if( parsefunc ) delete [] parsefunc;
	if( errorfunc ) delete [] errorfunc;
	if( parseargs ) delete [] parseargs;
	if( errprefix ) delete[] errprefix;
	if( ns ) delete[] ns;
	if( templatefile ) delete [] templatefile;
	if( templatedata ) delete [] templatedata;
}


// prints rule's action to stdout
void SourceGenerator::print_action( char *action, int rule, int expand_cpp, char *indent )
{
	char *m, *p, *l = action;
	char c;
...
void SourceGenerator::print_lexem_action( char *action, char *type, int expand_at, char *indent )
{
	int i;
	char *p, *l = action;
	char c;
...
// print part of code
void SourceGenerator::print_code( bool last, char *indent  )
{
	enum { SIZE = 16384, LINE = 4096};

	char *buffer = new char[SIZE], *maxline = buffer + SIZE;
        char lineindent[MAXINDENT];
        int firstline = 1; 
        int nskip = 0;
        int nindent = 0;
	char *p = buffer, *lastline = buffer, *p0 = buffer;

	while( l < end ) {
                if( firstline )
                {
                    if( *l && (*l == ' ' || *l == '\t') && nindent < MAXINDENT-1)
                    {
                        lineindent[nindent++] = *l;
                    }
                    else
                    {
                        lineindent[nindent] = 0;
                        firstline = 0;
                    }
                }
                if( *l == '\n' ) {
			...
			*p++ = *l++; nskip = 0; 
                        p0 = p;
			for( char *pfx = indent; *pfx; pfx++ ){ *p++ = *pfx; }
			lastline = p;

                } else if( *l == '\r' ) {
                    l++;
                } else if( nskip < nindent && *l == lineindent[nskip] ) {
                    l++; nskip++;
		} else {
                        nskip = nindent+1;
			*p++ = *l++;
...
{
	const char *p, *l = templatedata ? templatedata : langs[language].templ_gen;
	int action[16], deep = 0;
	const char *loop[16];
	int can_write = 1, denied_at = -1, line = 1;
	char var[128];
        char indent[MAXINDENT]={0};
        int nindent = 0;
	...
	while( *l ) {
		for( p = l; *l && *l != '@' && *l != '$'; l++ ) {
			if( *l == '\n' ) { indent[0] = 0; nindent=0; line++; }
                        else if( nindent < MAXINDENT-1 && (*l==' ' || *l=='\t') )
                        {
                            indent[nindent] = *l; nindent++; indent[nindent] = 0; 
                        }
                        else
                        {
                            nindent = MAXINDENT+1;
		...
		if( *l == '@' ) {
			...
			var[l-p] = 0;
			if(bracket) l++;
			if( print_variable( var, indent, can_write ) ) {
				for( ;*l && *l != '\n'; l++ );
				if( *l == '\n' ) l++, line++;
			}
...
int SourceGenerator::print_variable( char *var, char *indent, int can_write )
{
	int i, e;
	...
	switch( i ) {
		...
		case 4:  /* next */ 	 printf( "%s", (getsym)?getsym:"chr=next()" ); break;
		...
		case 15: /* lexem */  
			for( i = 0; i < lr.nstates; i++ ) {
				if( i ) printf( "%s%s", indent,  langs[language].lexem_start);
		...
		case 25: /* lexemactioncpp */
			if( iteration_counter >= 0 && iteration_counter < lr.nterms )
				print_lexem_action( lr.lact[iteration_counter], gr.sym[lr.lnum[iteration_counter]].type, 1, indent );
			else
				error( 0, "internal: using @%s in wrong loop\n", var );
			break;
		case 26: /* lexemaction */
			if( iteration_counter >= 0 && iteration_counter < lr.nterms )
				print_lexem_action( lr.lact[iteration_counter], gr.sym[lr.lnum[iteration_counter]].type, 0, indent );
			else
				error( 0, "internal: using @%s in wrong loop\n", var );
			break;
		case 27: /* rulenum */
		...
		case 28: /* ruleactioncpp */
			if( iteration_counter >= 0 && iteration_counter < gr.rules )
				print_action( gr.raction[iteration_counter], iteration_counter, 1, indent );
			else
				error( 0, "internal: using @%s in wrong loop\n", var );
			break;
		case 29: /* ruleaction */
			if( iteration_counter >= 0 && iteration_counter < gr.rules )
				print_action( gr.raction[iteration_counter], iteration_counter, 0, indent );
			else
				error( 0, "internal: using @%s in wrong loop\n", var );
			break;
		case 30: /* nativecode */
			print_code( false, indent );
			return 1;
		case 31: /* nativecodeall */
			print_code( true, indent );
			return 1;
		case 32: /* namespace */ 
			printf( "%s", (ns)?ns:"lapg" );
			break;
		case 33: /* tokenenum */
			for( i = 0; i < gr.nsyms; i++ )
				printf( "%s%s,\n", i?indent:"", sym_to_string(gr.sym[i].name, i) );
			return 1;
			break;
		case 34:  /* parsefunc */ printf( "%s", (parsefunc)?parsefunc:"parse" );break;
		case 35:  /* parseargs */ if( parseargs ) printf( "%s", parseargs );break;
		case 36:  /* errorfunc */ printf( "%s", (errorfunc)?errorfunc:"error" );break;
		case 37:  /* sourcename */ printf( "%s", (sourcename)?sourcename:"<unspecified source>" );break;
		case 38:  /* runtime */ 
                    {
                        time_t now;
                        time(&now);
                    ...
                    break;
		case 39:  /* lapgversion */ if( lapgversion ) printf( "%s", lapgversion );break;
...
{
	const char *s = templatedata ? templatedata : langs[language==-1?0:language].templ_gen;
	fwrite( s, 1, strlen(s), stdout );
}
